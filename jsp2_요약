...
- 설정 내용은  'https://mvnrepository.com/  -> 메이븐 저장소' 에서 검색합니다.
- 라이브러리 업데이트는 
프로젝트 선택 우클릭 - maven - update project : build를 다시합니다. 

4) 빌드란?(Build)
소스코드 작업하면서 디버깅/구현하는 단계에서 실행할수 있는 결과물을 만들어내는 과정.
...
예를 들면 pom.xml , mybatic-config.xml, member.xml 비교해보면 태그 형식이 다릅니다.
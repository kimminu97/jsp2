...
<%
	...
	long create = session.getCreationTime();	//세션이 만들어진 시간-처음요청을 보낸시간
	...
	out.print("<br>");
	/* 서버의 web.xml 설정 내용을 실행에 반영 -> 프로젝트의 web.xml 설정내용이 있으면 그것으로 변경
		<session-config>
        <session-timeout>30</session-timeout>
    </session-config>
    */
	
	long last = session.getLastAccessedTime();	//같은 세선ID로 요청한 최근시간
